import { 
  ...
  class NDIApiService {
    ...
    async createFoundationalIdProofRequest(): Promise<ProofRequestResult> {
      try {
        const response = await fetch(`${this.baseUrl}/Auth/ndi/request`, {
          method: 'POST',
          headers: { 
    ...
    async checkProof(threadId: string): Promise<ProofCheckResult> {
      try {
        const response = await fetch(`${this.baseUrl}/Auth/ndi/check/${threadId}`, {
          method: 'GET',
          headers: { 
            'Accept': 'application/json'
          }
    ...
    async registerWebhook(request: WebhookRegistrationRequest): Promise<any> {
      try {
        const response = await fetch(`${this.baseUrl}/ndi/webhook/register`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Accept': 'application/json'
    ...
    async subscribeThread(request: ProofSubscriptionRequest): Promise<any> {
      try {
        const response = await fetch(`${this.baseUrl}/ndi/subscribe`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Accept': 'application/json'
